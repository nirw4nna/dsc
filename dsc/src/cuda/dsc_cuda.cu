...
static DSC_INLINE void cast_op(const dsc_tensor *DSC_RESTRICT x,
    ...
    switch (out->dtype) {
        case BOOL: {
            DSC_DATA(bool, out);
            k_cast_op<Tx, bool><<<DSC_CUDA_BLOCKS(n),
                                 DSC_CUDA_DEFAULT_THREADS>>>(x_data, out_data, n);
            break;
        }
        case I32: {
            DSC_DATA(i32, out);
            k_cast_op<Tx, i32><<<DSC_CUDA_BLOCKS(n),
                                 DSC_CUDA_DEFAULT_THREADS>>>(x_data, out_data, n);
            break;
        }
        case BF16: {
            DSC_DATA(bf16, out);
            k_cast_op<Tx, bf16><<<DSC_CUDA_BLOCKS(n),
                                 DSC_CUDA_DEFAULT_THREADS>>>(x_data, out_data, n);
            break;
        }
        case F32: {
...
void dsc_cuda_arange(dsc_device *,
    ...
    switch (x->dtype) {
        case I32: {
            DSC_DATA(i32, x);
            k_assign_op<i32><<<DSC_CUDA_BLOCKS(n),
                               DSC_CUDA_DEFAULT_THREADS>>>(x_data, n, (i32) start, (i32) step);
            break;
        }
        case F32: {
            DSC_DATA(f32, x);
            k_assign_op<f32><<<DSC_CUDA_BLOCKS(n),
                               DSC_CUDA_DEFAULT_THREADS>>>(x_data, n, (f32) start, (f32) step);
            break;
        }
        case F64: {
            DSC_DATA(f64, x);
            k_assign_op<f64><<<DSC_CUDA_BLOCKS(n),
                               DSC_CUDA_DEFAULT_THREADS>>>(x_data, n, start, step);
            break;
        }
        DSC_INVALID_CASE("unknown dtype=%d", x->dtype);
    }
...
template <typename T>
static DSC_CUDA_KERNEL void k_repeat(const T *DSC_RESTRICT x,
                                     T *DSC_RESTRICT out,
...
void dsc_cuda_repeat(dsc_device *,
    ...
    switch (out->dtype) {
        case BOOL: {
            DSC_DATA(bool, x);
            DSC_DATA(bool, out);
            k_repeat<<<DSC_CUDA_BLOCKS(n), DSC_CUDA_DEFAULT_THREADS>>>(x_data, out_data,
                                                                       n, repeats,
                                                                       axis_idx, params);
            break;
        }
...
void dsc_cuda_transpose(dsc_device *,
    ...
    switch (out->dtype) {
        case BOOL: {
            DSC_DATA(bool, x);
            DSC_DATA(bool, out);
            k_transpose<bool><<<DSC_CUDA_BLOCKS(n),
        ...
        case I32: {
            DSC_DATA(i32, x);
            DSC_DATA(i32, out);
            k_transpose<i32><<<DSC_CUDA_BLOCKS(n),
                               DSC_CUDA_DEFAULT_THREADS>>>(x_data, out_data, n, params);
            break;
        }
        case F32: {
...
template <typename T>
static DSC_CUDA_KERNEL void k_tril(const T *DSC_RESTRICT x,
                                   T *DSC_RESTRICT out,
...
void dsc_cuda_tril(dsc_device *,
    ...
    switch (x->dtype) {
        case BOOL: {
            ...
            k_tril<<<DSC_CUDA_BLOCKS(n),
                     DSC_CUDA_DEFAULT_THREADS>>>(x_data, out_data, n, diagonal, params);
            break;
        }
        case I32: {
            DSC_DATA(i32, x);
...
void dsc_cuda_get_slice(dsc_device *,
    ...
    switch (out->dtype) {
        case BOOL: {
            DSC_DATA(bool, x);
            DSC_DATA(bool, out);
            k_get_slice<bool><<<DSC_CUDA_BLOCKS(n),
                               DSC_CUDA_DEFAULT_THREADS>>>(x_data, out_data, n, params);
            break;
        }
        case I32: {
            DSC_DATA(i32, x);
            DSC_DATA(i32, out);
            k_get_slice<i32><<<DSC_CUDA_BLOCKS(n),
                               DSC_CUDA_DEFAULT_THREADS>>>(x_data, out_data, n, params);
            break;
        }
        case F32: {
    ...
    if (xa_scalar) {
        ...
        switch (xa->dtype) {
            case BOOL: {
                DSC_DATA(bool, xa);
                DSC_DATA(bool, xb);
                k_set_slice<bool><<<DSC_CUDA_BLOCKS(n),
            ...
            case I32: {
                DSC_DATA(i32, xa);
                DSC_DATA(i32, xb);
                k_set_slice<i32><<<DSC_CUDA_BLOCKS(n),
                                   DSC_CUDA_DEFAULT_THREADS>>>(xa_data, xb_data, xb_scalar, n, params, whole);
                break;
            }
            case F32: {
...
static DSC_CUDA_KERNEL void k_binary_op(const Tx *xa, const Tx *xb, Tout *out,
                                        const int n, Op op,
...
static DSC_INLINE void binary_op(const dsc_tensor *xa,
    ...
    if (dsc_is_scalar(xa)) {
        k_binary_op<Tout, Tx, Op, true, false, false><<<DSC_CUDA_BLOCKS(n),
        ...
        k_binary_op<Tout, Tx, Op, false, true, false><<<DSC_CUDA_BLOCKS(n),
                                                        DSC_CUDA_DEFAULT_THREADS>>>(xa_data,
                                                                                    xb_data,
                                                                                    out_data,
                                                                                    n,
        ...
        k_binary_op<Tout, Tx, Op, false, false, true><<<DSC_CUDA_BLOCKS(n),
                                                        DSC_CUDA_DEFAULT_THREADS>>>(xa_data,
                                                                                    xb_data,
                                                                                    out_data,
                                                                                    n,
        ...
        k_binary_op<Tout, Tx, Op, false, false, false><<<DSC_CUDA_BLOCKS(n),
                                                         DSC_CUDA_DEFAULT_THREADS>>>(xa_data,
                                                                                     xb_data,
                                                                                     out_data,
                                                                                     n,
...
void dsc_cuda_matmul(dsc_device *dev,
    ...
    switch (xa->dtype) {
        case F32: {
            ...
            for (int d0 = 0; d0 < d0_out; ++d0) {
                for (int d1 = 0; d1 < d1_out; ++d1) {
                    ...
                    DSC_CUBLAS_FAIL_ON_ERROR(cublasSgemm(info->cublas_handle,
                                                         trans_b ? CUBLAS_OP_T : CUBLAS_OP_N, CUBLAS_OP_N, n, m, k,
        ...
        case F64: {
            ...
            static constexpr f64 alpha = 1.f, beta = 0.f;
            ...
            for (int d0 = 0; d0 < d0_out; ++d0) {
                for (int d1 = 0; d1 < d1_out; ++d1) {
                    ...
                    DSC_CUBLAS_FAIL_ON_ERROR(cublasDgemm(info->cublas_handle,
                                                         trans_b ? CUBLAS_OP_T : CUBLAS_OP_N, CUBLAS_OP_N, n, m, k,
                                                         &alpha, &xb_data[xb_offset], stride_b,
                                                         &xa_data[xa_offset], stride_a, &beta,
                                                         &out_data[out_offset], stride_out));
...
void dsc_cuda_compare(dsc_device *,
                      const dsc_tensor *xa,
                      const dsc_tensor *xb,
    ...
    switch (comp) {
        case EQ:
            binary_op(xa, xb, out, cuda_eq_op());
            break;
        case NE:
            binary_op(xa, xb, out, cuda_ne_op());
            break;
        case LT:
            binary_op(xa, xb, out, cuda_lt_op());
            break;
        case LE:
            binary_op(xa, xb, out, cuda_le_op());
            break;
        case GT:
            binary_op(xa, xb, out, cuda_gt_op());
            break;
        case GE:
            binary_op(xa, xb, out, cuda_ge_op());
            break;
...
template <typename T>
static DSC_CUDA_KERNEL void k_masked_fill(T *DSC_RESTRICT x,
...
void dsc_cuda_masked_fill(dsc_device *,
                          dsc_tensor *DSC_RESTRICT x,
    ...
    switch (x->dtype) {
        case BOOL: {
            ...
            k_masked_fill<<<DSC_CUDA_BLOCKS(n),
                            DSC_CUDA_DEFAULT_THREADS>>>(x_data, mask_data, n,
                                                        (bool) value, params);
            break;
        }
        case I32: {
...
template <typename T>
static DSC_CUDA_KERNEL void k_outer(const T *DSC_RESTRICT xa,
                                    const T *DSC_RESTRICT xb,
                                    T *DSC_RESTRICT out,
                                    const int n,
    ...
    for (int i = tid; i < n; i += stride) {
        ...
        out[i] = cuda_mul_op()(xa[ii], xb[jj]);
    }
}

void dsc_cuda_outer(dsc_device *,
                    const dsc_tensor *DSC_RESTRICT xa,
                    const dsc_tensor *DSC_RESTRICT xb,
                    dsc_tensor *DSC_RESTRICT out) {
    const int n = out->ne;
    ...
    switch (xa->dtype) {
        case BOOL: {
            ...
            k_outer<<<DSC_CUDA_BLOCKS(n),
                      DSC_CUDA_DEFAULT_THREADS>>>(xa_data, xb_data, out_data, n, params);
            break;
        }
        case I32: {
            DSC_DATA(i32, xa);
...
template <typename T>
static DSC_CUDA_KERNEL void k_where(const bool *DSC_RESTRICT condition,
...
void dsc_cuda_where(dsc_device *,
    ...
    switch (out->dtype) {
        case BOOL: {
            ...
            k_where<<<DSC_CUDA_BLOCKS(n),
                      DSC_CUDA_DEFAULT_THREADS>>>(condition_data, input_data,
...
void dsc_cuda_tanh(dsc_device *,
                 const dsc_tensor *DSC_RESTRICT x,
                 dsc_tensor *DSC_RESTRICT out) {
    unary_op(x, out, cuda_tanh_op());
}
...
static DSC_CUDA_KERNEL void k_reduce(const T *DSC_RESTRICT x,
                                     T *DSC_RESTRICT out,
...
static DSC_INLINE void reduce_op(const dsc_tensor *DSC_RESTRICT x,
    ...
    k_reduce<<<DSC_CUDA_BLOCKS(n),
               DSC_CUDA_DEFAULT_THREADS>>>(x_data, out_data, default_value,
...
void dsc_cuda_sum(dsc_device *,
    ...
    switch (out->dtype) {
        case F32:
            reduce_op<f32, cuda_add_op, cuda_atomic_add_op>(x, out, axis_idx, dsc_zero<f32>(),
                                                            cuda_add_op(), cuda_atomic_add_op());
            break;
        case F64:
            reduce_op<f64, cuda_add_op, cuda_atomic_add_op>(x, out, axis_idx, dsc_zero<f64>(),
                                                            cuda_add_op(), cuda_atomic_add_op());
            break;
        DSC_INVALID_CASE("unknown dtype=%d", out->dtype);
    }
...
void dsc_cuda_min(dsc_device *,
                  const dsc_tensor *DSC_RESTRICT x,
                  dsc_tensor *DSC_RESTRICT out,
                  const int axis_idx) {
    switch (out->dtype) {
        case F32:
            reduce_op<f32, cuda_min_op, cuda_atomic_min_op>(x, out, axis_idx, dsc_inf<f32, true>(),
                                                            cuda_min_op(), cuda_atomic_min_op());
            break;
        case F64:
            reduce_op<f64, cuda_min_op, cuda_atomic_min_op>(x, out, axis_idx, dsc_inf<f64, true>(),
                                                            cuda_min_op(), cuda_atomic_min_op());
            break;
        DSC_INVALID_CASE("unknown dtype=%d", out->dtype);
    }
...
void dsc_cuda_max(dsc_device *,
                  const dsc_tensor *DSC_RESTRICT x,
                  dsc_tensor *DSC_RESTRICT out,
                  const int axis_idx) {
    switch (out->dtype) {
        case F32:
            reduce_op<f32, cuda_max_op, cuda_atomic_max_op>(x, out, axis_idx, dsc_inf<f32, false>(),
                                                            cuda_max_op(), cuda_atomic_max_op());
            break;
        case F64:
            reduce_op<f64, cuda_max_op, cuda_atomic_max_op>(x, out, axis_idx, dsc_inf<f64, false>(),
                                                            cuda_max_op(), cuda_atomic_max_op());
            break;
        DSC_INVALID_CASE("unknown dtype=%d", out->dtype);
    }
